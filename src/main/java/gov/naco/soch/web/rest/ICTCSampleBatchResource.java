...
public class ICTCSampleBatchResource {
    ...
    @Autowired
    ICTCSampleBatchQueryService iCTCSampleBatchQueryService;


    @PostMapping("/ictc-sample-batches")
    public ResponseEntity<ICTCSampleBatchDTO> createICTCSampleBatch(@Valid @RequestBody CreateSampleBatchDTO batchDetails) throws URISyntaxException {
        log.debug("REST request to save ICTCSampleBatch : {}", batchDetails);
        ICTCSampleBatchDTO result = iCTCSampleBatchService.createBatch(batchDetails);
    ...
    public ResponseEntity<ICTCSampleBatchDTO> getICTCSampleBatch(@PathVariable Long id) {
        ...
        return ResponseEntity.of(iCTCSampleBatchDTO);
    }
