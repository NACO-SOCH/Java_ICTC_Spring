...
//	
	@Query("select ib from ICTCBeneficiary ib where ib.pid=:pid and (ib.deleteReason != 5 or ib.deleteReason is null)")
	Optional<ICTCBeneficiary> findByPidAndDeleteReasonNot(@Param("pid") String pid);
	
	Long countByInfantCode(String infantCode);
	
	@Query("select ib from ICTCBeneficiary ib where ib.infantCode=:infantCode and (ib.deleteReason != 5 or ib.deleteReason is null)")
    Optional<ICTCBeneficiary> findByInfantCodeAndDeleteReasonNot(@Param("infantCode") String infantCode);
